	...
	public class LoginCmsClient
	{
													...
													bool verbose)
		{
            ...
            if (File.Exists(ticketCacheFile))
            {
                ...
                if (DateTime.UtcNow <= ticket.ExpirationTime) { return ticket; }
            }
            */
			...
			{
				throw new Exception(ID_FNC + " Path: " + CertificatePath + " ***Error GENERANDO el LoginTicketRequest : " + ex.Message + ex.StackTrace);
			}
			...
			{
				throw new Exception(ID_FNC + " Path: " + CertificatePath + " ***Error FIRMANDO el LoginTicketRequest : " + ex.Message);
			}
